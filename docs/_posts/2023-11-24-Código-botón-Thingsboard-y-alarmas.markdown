...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    resp["Temperatura 1"] = 50; ////Cambiar value por temp/1 
    serializeJson(resp, buffer);
    ...
    resp["Humedad 1"] = value; //Cambiar value por humedad1
    serializeJson(resp, buffer);
    mqttClient.publish("v1/devices/me/telemetry", buffer);
    Serial.print("Publish Message [telemetry]: ");
    Serial.println(buffer);
...
private:
    ...
    void inicializarDatosDePlantas() {
        // Inicializando datos de plantas con rangos de temperatura y humedad
        // Formato: datosDePlantas["NombrePlanta"] = {{tempMin, tempMax}, {humedadMin, humedadMax}};
...
public:
    Auxiliar(const std::string& planta) {
        ...
        if (it != datosDePlantas.end()) {
            ...
            // Handle the error without throwing an exception
            Serial.println("Planta desconocida");
            // You might want to take some action here, e.g., set a default plant
    ...
    bool esTemperaturaAdecuada(int temp) {
        ...
        bool result = !(temp >= rango.first && temp <= rango.second);
        return result;
    }
...
void handleAndPublishTemperature(int sensorIndex, const char* telemetryTopic, const char* attributeTopic) {
    ...
    bool isTempSuitable = miPlanta.esTemperaturaAdecuada(temperature);
    Serial.print("Temperature ");
    Serial.print(sensorIndex);
    Serial.print(": ");
    Serial.println(temperature);
    Serial.print("Is Suitable: ");
    Serial.println(isTempSuitable);

    resp.clear();
    resp["estado_fan"] = isTempSuitable;
    serializeJson(resp, buffer);
    mqttClient.publish(attributeTopic, buffer);
}
...
void loop() {
    ...
    if (now - lastMsg > 2000) {
        ...
        handleAndPublishTemperature(0, "v1/devices/me/telemetry/sensor1", "v1/devices/me/attributes/sensor1");

        // Handle thermometer 2
        handleAndPublishTemperature(1, "v1/devices/me/telemetry/sensor2", "v1/devices/me/attributes/sensor2");

        // Handle humidity sensor 1
        //handleAndPublishHumidity()
